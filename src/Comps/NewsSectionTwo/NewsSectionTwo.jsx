  ...
  return (
    <div>
      <div className="row gx-3">
        {binodon.map(({ id, title_img, news_title, news_short_brief, category_name_bangla }) => (
            ...
            <Link to={`/${category_name_bangla}/${id}`} className="text-muted">
              <div className="card border-0 shadow-sm mb-3">
                <LazyImageNews
                  ...
                  style={{ width: "50px", height: "50px", objectFit: "cover", borderRadius: "0.25rem", overflow: "hidden" }}
                />
                <div className="card-body" style={{ height: "9rem" }}>
                  <h6 className="mb-0 main-color" style={{ fontSize: "20px" }}>
                    <SanitizedParagraph className="mb-0" style={{ marginBottom: "0px" }} htmlContent={news_title.split(" ").slice(0, 5).join(" ")} />
                  </h6>
                  <p className="m-0 pt-1">
                    <SanitizedParagraph htmlContent={news_short_brief.split(" ").slice(0, 5).join(" ")} />
                  </p>
                </div>
              </div>
            </Link>
      ...
      <div className="row">
        <div className="col-lg-12">
          {advertisementBinodon.map((data) =>
            data.status === 1 && data.ad_position === "BelowNewsCategory2" && (
